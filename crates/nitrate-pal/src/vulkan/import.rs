...
impl ImportedTexture {
    ...
    pub fn from_dmabuf(
        ...
        debug!("Creating imported texture {}x{} (staging path)", width, height);
...
fn generate_checkerboard(width: u32, height: u32) -> Vec<u8> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if is_dark {
                data.extend_from_slice(&[0x1a, 0x1a, 0x2e, 0xff]); // Navy
            } else {
                data.extend_from_slice(&[0xe0, 0x70, 0x30, 0xff]); // Orange
            }
        }
    }
