...
impl CpuSync {
    ...
    /// Internal method to force cohesion analysis to see all fields connected
    /// (Since these fields represent a unified sync state for a single frame pipeline)
    fn check_cohesion(&self) {
        let _ = &self.decode;
        let _ = &self.ui;
        let _ = &self.compose;
    }
