// ===== START OF FILE: webphy/web/modules/handheld-camera.js ===== //
...
export class HandheldCameraModule {
  ...
  createProgram() {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error(
        "Handheld shake program link error:",
  ...
  apply(inputTex, outputFB, state, frameSeed, canvasW, canvasH) {
    ...
    const yWobble = layeredPerlin(
      time * wobbleFreq * 0.9 + this.seeds.yLow,
      0,
      2,
    );
    const rotWobble = layeredPerlin(
      time * wobbleFreq * 0.85 + this.seeds.rotLow,
      0,
      2,
    );
    ...
    const yJitter = layeredPerlin(
      time * jitterFreq * 1.08 + this.seeds.yHigh,
      0,
      3,
    );
    const rotJitter = layeredPerlin(
      time * jitterFreq + this.seeds.rotHigh,
      0,
      3,
    );
    ...
    const offsetY =
      (yWobble * wobble * 0.6 + yMid * 0.3 + yJitter * jitter * 0.1) *
      baseTransY *
      intensity;
    ...
    gl.uniform2f(
      gl.getUniformLocation(this.program, "uShakeOffset"),
      offsetX,
      offsetY,
    );
...
// ===== END OF FILE: webphy/web/modules/handheld-camera.js ===== //
